...
namespace NatCorder {
    ...
    public sealed class MP4Recorder : IMediaRecorder {
        ...
        [Doc(@"MP4RecorderCtor")]
        public MP4Recorder (int width, int height, float framerate, int sampleRate, int channelCount, Action<string> recordingCallback, int bitrate = (int)(960 * 540 * 11.4f), int keyframeInterval = 3, string recName = "recording") {
            var recordingDirectory = Application.persistentDataPath;
            var recordingName = string.Format("{0}_{1}.mp4", recName, DateTime.Now.ToString("HH-mm-ss"));
