...
public class vThirdPersonCamera : MonoBehaviour
{
    ...
    bool CullingRayCast(Vector3 from, ClipPlanePoints _to, out RaycastHit hitInfo, float distance, LayerMask cullingLayer, Color color)
    {
        ...
        Ray ray = new Ray(from, _to.LowerLeft - from);
        if (DiggerPhysics.Raycast(ray, out hitInfo, distance))
        {
            value = true;
            cullingDistance = hitInfo.distance;
        }

        ray = new Ray(from, _to.LowerRight - from);
        if (DiggerPhysics.Raycast(ray, out hitInfo, distance))
        {
            value = true;
        ...
        ray = new Ray(from, _to.UpperLeft - from);
        if (DiggerPhysics.Raycast(ray, out hitInfo, distance))
        {
            value = true;
            if (cullingDistance > hitInfo.distance) cullingDistance = hitInfo.distance;
        ...
        ray = new Ray(from, _to.UpperRight - from);
        if (DiggerPhysics.Raycast(ray, out hitInfo, distance))
        {
            value = true;
            if (cullingDistance > hitInfo.distance) cullingDistance = hitInfo.distance;
