...
namespace Invector.CharacterController
{
    public abstract class vThirdPersonMotor : MonoBehaviour
    {
        ...
        public AnimationCurve slipageCurve;
        float slipage;
        ...
        protected void ControlSpeed(float velocity)
        {
            ...
            else
            {
                ...
                if (DiggerPhysics.Raycast(ray, out hitinfo, 1f))
                {
                    groundAngleTwo = Vector3.Angle(Vector3.up, hitinfo.normal);
                    Vector3 slopeDirection = Vector3.Cross(Vector3.up, Vector3.Cross(Vector3.up, hitinfo.normal));
                    slope = Vector3.Dot(transform.forward, slopeDirection);
                }

                if (slope > 0.01f)
                {
                    slipage = HLP.Map(groundAngleTwo, slopeLimit - modRange, slopeLimit, 0f, 1f);
                    slipage = slipageCurve.Evaluate(slipage);
                }
                // Debug.Log("Slip: " + slipage + " - Angle: " + groundAngleTwo + "Slope: " + slope);

                var velY = transform.forward * velocity * speed * (1f - slipage);
        ...
        protected bool jumpFwdCondition
        {
            get
            {
                ...
                RaycastHit[] output = Physics.CapsuleCastAll(p1, p2, _capsuleCollider.radius * 0.5f, transform.forward, 0.6f, groundLayer);

                foreach (RaycastHit item in output)
                {
                    if (
                        item.collider.CompareTag("Player") || 
                        item.collider.CompareTag("PlayerCollider") ||
                        item.collider.name.Equals("ROI_Trigger")
                        ) continue;
        ...
        void CheckGroundDistance()
        {
            if (_capsuleCollider != null)
            {
                ...
                float radius = _capsuleCollider.radius * 1.9f;
                var dist = 20f;
                ...
                if (DiggerPhysics.Raycast(ray1, out groundHit, colliderHeight / 2 + 2f))
                ...
                groundDistance = (float)System.Math.Round(dist, 3);
                groundDistance = Mathf.Max(groundDistance, 0f);
            }
        }
        ...
        bool StepOffset()
        {
            if (input.sqrMagnitude < 0.1 || !isGrounded || slipage == 1f) return false;
            ...
            if (DiggerPhysics.Raycast(rayStep, out _hit, stepOffsetEnd - stepOffsetStart) && !_hit.collider.isTrigger)
            {
